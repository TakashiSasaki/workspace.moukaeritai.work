...
description: >
  ...
  Interactions API for the Antigravity agent, Deep Research agent, and general
  model capabilities (text, image, speech generation), as well as other specialized
  APIs for video (Veo), music (Lyria), and real-time audio (Live API) in a server-side
  context.
  Use when user requests the Interactions API, Antigravity agent, or Deep Research
  agent.
---
...
-   **Agents require Interactions API**: Managed agents (such as
    `antigravity-preview-05-2026` or `deep-research-preview-04-2026`) and custom
    agents can **ONLY** be invoked using the `ai.interactions.create()` method.
    They **cannot** be invoked using `ai.models.generateContent()` or
    `ai.models.generateContentStream()`. Calling a model using `generateContent`
    is only for standard models (like `gemini-3.5-flash`), not agents.
...
// Return operation.name to the client for polling.
// The client polls the backend, which calls getVideosOperation
// and downloads the video on the client's behalf.
```
...
#### Server-Side Video Rules

Video generation is a long-running operation. The frontend must never see the
API key or download URI. Use this 3-step POST pattern:

To poll or download, reconstruct a `GenerateVideosOperation` from the operation
name. The SDK only reads the `name` property internally, so a minimal instance
is sufficient — no need to store the full SDK object.
...
1.  **Start** (`POST /api/generate-video`) — backend calls `generateVideos` and
    returns only the `name` to the client. The `name` follows the format
    `models/{model}/operations/{id}`.
...
2.  **Poll** (`POST /api/video-status`) — client sends `{ operationName }` in
    the body. Backend reconstructs an operation and polls:
...
3.  **Download** (`POST /api/video-download`) — client sends `{ operationName }`
    in the body. Backend polls the completed operation, extracts the download
    URI, fetches the video with the API key, and streams it back:
    ...
    const videoRes = await fetch(uri, {
      headers: { 'x-goog-api-key': apiKey }, // apiKey is the Gemini API key
    });
